...
Communicates with the Central Supervisor over HTTP.
"""
...
class WorkerDaemon:
    """Daemon running on a worker node, reporting heartbeat and polling tasks."""
    ...
    def register(self) -> bool:
        ...
        try:
            resp = requests.post(self.api_url("/workers"), json=payload, timeout=5.0)
    ...
    def execute_task(self, task: Dict[str, Any]) -> None:
        ...
        try:
            requests.put(self.api_url(f"/tasks/{task_id}/start"), json={"worker_id": self.worker_id}, timeout=5.0)
        except Exception as e:
        ...
        print(f"[{self.worker_id}] Processing instructions:\n{prompt}")

        # Worker simulation / execution hook
        time.sleep(2.0)

        # Report completion
        result_summary = f"Task completed successfully by {self.worker_id} on {self.machine}"
        try:
            resp = requests.post(
    ...
    def run(self) -> None:
        """Main daemon loop."""
        self.running = True
        print(f"Starting Worker Daemon {self.worker_id} [{self.environment}]...")
